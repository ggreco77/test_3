...
class UserValuesGUI:
    """GUI with 8 labelframe widgets:
        ...
        5. FoV shape
        6. Query
        7. Statistic window/Trasparency
    ...
    def __init__(self, master):
        ...
        self.is_collapse = False # init collapse method
        ...
        self.seven_label_frame = LabelFrame(master,
                                          text=" 7. Statistic window/Trasparency") 
...
##        self.browse_1.grid(row=0, column=8, sticky='W', padx=5, pady=2)
        ...
        self.show_aladin_1 = Button(self.one_label_frame,
                                    text="Load in Aladin",command=self.show_healpix_skymap)
...
##        self.browse_1.grid(row=1, column=8, sticky='W', padx=5, pady=2)
        ...
        self.show_collapse = Button(self.two_label_frame,
                                    text="᎐", command=self.collapse)
        self.show_collapse.grid(row=2, column=60, sticky='E', padx=5, pady=2)

        #self.show_intersection = Button(self.two_label_frame,
        #                            text="᎒",command=self.moc_intersection)
        #self.show_intersection.grid(row=2, column=80, sticky='E', padx=5, pady=0)
        ...
        column_filter_1_default = StringVar(self.six_label_frame, value=">100&<200")   # default
...
##        
##        try:
##            infile  = gzip.GzipFile(self.entry_skymap.get(),'rb').read()
...
##                                    samphub_error)
    ...
    def probability_contours(self):
        ...
        try:
            ...
            messagebox.showerror ('Error: json contour file', io_error)
            ...
            try:
                ...
                messagebox.showerror ('Error: json contour file', samphub_error)

    def moc_contour(self):
        """Creating & Showing MOC plots in a folder."""
        ...
        _get_last = 0.0001 # getting last number in _to 
        
        _from = float(self.entry_from.get())
        _to = float(self.entry_to.get()) + _get_last 
        _step = float(self.entry_step.get())

        assert (_to > 0 and _to <100 and _from > 0 and _step > 0), "Insert a valid input"
        
        # better two "for cicles" in Aladin
        for i in np.arange(_from ,_to, _step):        
            aladin.cmoc((i/100.0), self.entry_skymap.get(),'moc'+str(i/100.0) + suffix)

        for i in np.arange(float(self.entry_from.get()),(float(self.entry_to.get()) + _get_last),
                       float(self.entry_step.get())):
            
            aladin.set_moc('moc'+str(i/100.0)+ suffix)
            aladin.mv('moc'+str(i/100.0)+ suffix,'MOC' + suffix)

    def collapse(self):
        """Collapsing and expanding MOC folder."""
        
        if self.is_collapse == False:
            self.show_collapse.configure(text="᎒")
    ...
    def moc_intersection(self):
        ...
        aladin.send_file( 'VST_MOC.fits' )  # sending to Aladin
